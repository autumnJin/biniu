        ...
        <div class="am-u-sm-12 am-u-md-6">
          <div class="am-btn-toolbar">
            <div class="am-btn-group am-btn-group-xs">
              <a href="<?php echo U('goods/add_goods');?>" type="button" class="am-btn am-btn-primary"><span class="am-icon-plus"></span> 新增</a>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="am-u-sm-12">
          <form class="am-form">
            <table class="am-table am-table-striped am-table-hover table-main">
              <thead>
              <tr>
                <th class="table-id">货号</th>
                <th class="table-title">名称</th>
                <th class="table-title">总库存</th>
                <th class="table-title">已售</th>
                <th class="table-title">logo</th>
                <th class="table-type">商品售价</th>
                <!--<th class="table-type">类型</th>-->
                <!--<th class="table-type">可见性</th>-->
                <th class="table-set">是否上架</th>
                <th class="table-set">创建时间</th>
                <th class="table-set">操作</th>
              </tr>
              </thead>
              <?php $goods_type = C('goods_type'); ?>
              <tbody>
              <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                  <td><?php echo ($vo['id']); ?></td>
                  <td><?php echo ($vo['name']); ?></td>
                  <th><?php echo ($vo["repertory"]); ?></th>
                  <th><?php echo ($vo["sell"]); ?></th>
                  <td><img src="<?php echo ($vo["logopath"]); ?>" style="width: 40px;height: 40px"></td>
                  <td><?php echo ($vo['price']); ?></td>
                  <!--<td><?php echo ($goods_type[$vo['pro_type']]); ?></td>-->


                  <td><?php echo $vo['status'] == 1?'已上架':'已下架'; ?></td>
                  <td><?php echo (date("Y-m-d H:i:s",$vo['create_time'])); ?></td>
                  <?php $text = $vo['status'] == 1?'下架此商品':'上架此商品'; ?>
                  <td>
                    <div class="am-btn-toolbar">
                      <div class="am-btn-group am-btn-group-xs">
                        <a href="<?php echo U('edit_goods',array('id'=>$vo['id']));?>" class="am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span> 编辑</a>
                        <button class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only xiajia"  g="<?php echo ($vo['id']); ?>"><span class="am-icon-trash-o"></span>删除</button>
                        <button class="am-btn am-btn-default am-btn-xs  am-hide-sm-only xx"  g="<?php echo ($vo['id']); ?>" s="<?php echo ($vo['status']); ?>"><span class="am-icon-trash-o"></span><?php echo ($text); ?></button>
                      </div>
                    </div>
                  </td>
  ...
  <script>
    $(function () {
        $('.xiajia').click(function () {
            ...
            var url = "<?php echo U('del_goods');?>";
            var g_id = $(this).attr('g');
            $.post(url,{id:g_id},function (data) {
                alert(data.msg);
                if(data.code){
                    return;
                }else{
        ...
        $('.xx').click(function () {
            ...
            var url = "<?php echo U('xiajia_goods');?>";
            var g_id = $(this).attr('g');
            var status = $(this).attr('s');
            $.post(url,{id:g_id,status:status},function (data) {
                alert(data.msg);
                if(data.code){
                    return;
                }else{
