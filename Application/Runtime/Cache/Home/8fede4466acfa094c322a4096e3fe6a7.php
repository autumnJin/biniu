...
<body>
  ...
  <section class="zhuanzhang">
    <div class="zhuanzhang_choose">
      <div>
        ...
        <img src="https://bbtcdn.8btc.com/data/attachment/common/c8/common_2_icon.svg" alt="">
        <span>WFX</span><span style="margin-left: 0.5rem;">剩余数量<?php echo ($sell["lave"]); ?>，最低数量<?php echo ($sell["low_num"]); ?></span>
      </div>
      <div>
    ...
    <div class="zhuanzhang_input">
      ...
      <section>
        <p>买入数量</p>
        <input type="text" placeholder="填写买入数量" name="amount">
      </section>
        <!--<section>-->
          <!--<p>最低数量</p>-->
      ...
      <section>
        ...
        <input id="sell_id" value="<?php echo ($sell["id"]); ?>" name="sell_id" type="hidden"/>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
            <div class="record">
              ...
              <div class="record_list_content">
                ...
                <p>时间：<?php echo (date('Y-m-d H:i:s',$vo["buy_time"])); ?></p>

                <!--<p>挂售数量：<?php echo ($vo["sell_num"]); ?> WFX</p>-->
                <p>已买入数量：<?php echo ($vo["buy_num"]); ?> WFX</p>
  ...
  <script>
    $("#btn").click(function () {
      $.ajax({
        type:"post",
        url:"<?php echo U('Otc/buy');?>",
        data: $("#cp").serialize(),
        dataType:"json",
        success:function(data){
          if(data.code > 0){
            alert(data.message);
            window.location.href = "<?php echo U('Otc/buy');?>";
          }else{
            alert(data.message);
          }
        }
